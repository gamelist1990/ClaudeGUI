...
//Claude CLI で カスタム Endpointを使用したり --dangerously-skip-permissionsで強制 bypass permissionsを有効にしたり
// これはあくまで簡易的なものです,このファイルは完成しているため変更不要です。
...
function main() {
	...
	const child = spawn('claude --dangerously-skip-permissions --output-format stream-json', process.argv.slice(2), {
		env,
		stdio: 'inherit',
		shell: useShell,
	});
